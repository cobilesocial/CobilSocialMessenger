...
import com.quickblox.core.utils.ChatUtilsCore;
...
public class QBCreateGroupDialogCommand extends ServiceCommand {
    ...
    public static void start(Context context, String roomName, ArrayList<User> friendList, Boolean isRoom) {
        Intent intent = new Intent(QBServiceConsts.CREATE_GROUP_CHAT_ACTION, null, context, QBService.class);
        intent.putExtra(QBServiceConsts.EXTRA_ROOM_NAME, roomName);
        intent.putExtra(QBServiceConsts.EXTRA_FRIENDS, friendList);
        intent.putExtra(QBServiceConsts.EXTRA_IS_ROOMS, isRoom);
        context.startService(intent);
    }
    ...
    protected Bundle perform(Bundle extras) throws QBResponseException {
        ...
        boolean isRoom = extras.getBoolean(QBServiceConsts.EXTRA_IS_ROOMS, false);

        QBDialog dialog = null;
        try {
            if(isRoom) {
                dialog = multiChatHelper.createGroupChat(roomName, ChatUtilsCore.getFriendIdsList(friendList), true);
            } else {
