...
import com.quickblox.socialmessenger.utils.TextViewHelper;
import com.quickblox.core.db.managers.UsersDatabaseManager;
import com.quickblox.core.models.Friend;
            ...
            for (FriendGroup friendGroup : originalList) {
                ...
                for (User user : userList) {
                    if (user.getFullName().toLowerCase().contains(query) || user.getFullName()
                            .toLowerCase().contains(query) || (user.getLogin() != null && user.getLogin().toLowerCase().contains(query))) {
