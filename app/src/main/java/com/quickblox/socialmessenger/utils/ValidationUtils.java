...
public class ValidationUtils extends SimpleTextWatcher {
    ...
    public boolean isValidUserDate(String emailText, String passwordText) {
        boolean isEmailValid = isValidUsername(fieldsArray[0], emailText);
    ...
    public boolean isValidUsername(EditText usernameEditText, String usernameText){
        boolean onlyCorrectSymbols;
        Pattern p = Pattern.compile("^[-A-Za-z0-9\\]\\[\\!\\\"\\#\\$\\%\\&\\'\\(\\)\\*\\+\\,\\.\\/\\:\\;\\<\\=\\>\\?\\@\\\\\\^\\_\\`\\{\\|\\}\\~]+");
        Matcher m = p.matcher(usernameText);
        onlyCorrectSymbols = m.matches();

        if (!onlyCorrectSymbols){
            usernameEditText.setError(context.getString(R.string.error_please_enter_a_valid_username_from_app));
        }

        return onlyCorrectSymbols;
    }
